...
with tabs[3]:
    ...
    st.set_page_config(
        page_title="User Engagement Analysis",
        page_icon="📊",
        layout="wide"
    )
    ...
    st.markdown("""
    This dashboard analyzes user engagement patterns and predicts highly engaged users using XGBoost.
    """)
    
    # Display raw data sample in an expander
    with st.expander("View Raw Data Sample", expanded=False):
        st.dataframe(actions_avec_score_df.head())
    
    # Show basic statistics
    st.subheader("Data Overview")
    col1, col2, col3, col4 = st.columns(4)
    col1.metric("Total Users", len(actions_avec_score_df['action_user_name'].unique()))
    col2.metric("Total Actions", len(actions_avec_score_df))
    col3.metric("Median Engagement Score", round(actions_avec_score_df['score_engagement'].median(), 2))
    col4.metric("Average Engagement Score", round(actions_avec_score_df['score_engagement'].mean(), 2))
    ...
    fig = px.histogram(actions_avec_score_df, x="score_engagement", 
                      color="engaged", 
                      marginal="box", 
                      nbins=30,
                      labels={"score_engagement": "Engagement Score", "engaged": "Engaged"},
                      color_discrete_sequence=["#75AADB", "#FF5A5F"])
    
    fig.add_vline(x=median_threshold, line_dash="dash", line_color="red", 
                 annotation_text=f"Median: {median_threshold:.2f}")
    ...
    with st.expander("View Aggregated User Data"):
        st.dataframe(user_df)
    
    # Feature correlation heatmap
    st.subheader("Feature Correlation")
    corr = user_df.drop(columns=['action_user_name']).corr()
    fig, ax = plt.subplots(figsize=(10, 8))
    sns.heatmap(corr, annot=True, cmap='coolwarm', fmt=".2f", linewidths=0.5, ax=ax)
    st.pyplot(fig)
    
    # Feature importance visualization
    st.subheader("User Engagement Model")
    ...
    # Model metrics
    col1, col2, col3 = st.columns(3)
    col1.metric("Accuracy", f"{metrics.accuracy_score(y_test, y_pred):.3f}")
    col2.metric("Precision", f"{metrics.precision_score(y_test, y_pred):.3f}")
    col3.metric("Recall", f"{metrics.recall_score(y_test, y_pred):.3f}")
    ...
    st.write("Select values to predict if a user would be highly engaged:")
    
    col1, col2 = st.columns(2)
    with col1:
        nb_actions = st.slider("Number of Actions", 1, int(user_df['nb_actions'].max()), 10)
        nb_sessions = st.slider("Number of Sessions", 1, int(user_df['nb_sessions'].max()), 3)
        nb_jours_actifs = st.slider("Days Active", 1, int(user_df['nb_jours_actifs'].max()), 2)
        engagement_moyen = st.slider("Average Engagement", float(user_df['engagement_moyen'].min()), float(user_df['engagement_moyen'].max()), float(user_df['engagement_moyen'].mean()))
        engagement_max = st.slider("Max Engagement", float(user_df['engagement_max'].min()), float(user_df['engagement_max'].max()), float(user_df['engagement_max'].mean()))
        engagement_min = st.slider("Min Engagement", float(user_df['engagement_min'].min()), float(user_df['engagement_max'].min()), float(user_df['engagement_min'].mean()))
    
    with col2:
        part_nouvelles_visites = st.slider("Proportion of New Visits", 0.0, 1.0, 0.5)
        part_visites_recurrentes = st.slider("Proportion of Recurring Visits", 0.0, 1.0, 0.5)
        poids_moyen_actions = st.slider("Average Action Weight", float(user_df['poids_moyen_actions'].min()), float(user_df['poids_moyen_actions'].max()), float(user_df['poids_moyen_actions'].mean()))
        nb_groupes_uniques = st.slider("Number of Unique Groups", 1, int(user_df['nb_groupes_uniques'].max()), 2)
        nb_labels_uniques = st.slider("Number of Unique Labels", 1, int(user_df['nb_labels_uniques'].max()), 3)
        nb_actions_par_jour = st.slider("Actions per Day", float(user_df['nb_actions_par_jour'].min()), float(user_df['nb_actions_par_jour'].max()), float(user_df['nb_actions_par_jour'].mean()))
    ...
    input_data = pd.DataFrame({
        ...
        'part_nouvelles_visites': [part_nouvelles_visites],
        'part_visites_recurrentes': [part_visites_recurrentes],
        'poids_moyen_actions': [poids_moyen_actions],
        'nb_groupes_uniques': [nb_groupes_uniques],
        'nb_labels_uniques': [nb_labels_uniques],
        'nb_actions_par_jour': [nb_actions_par_jour]
    })
    ...
    if st.button("Predict Engagement"):
        ...
        if prediction == 1:
            st.success(f"This user is likely to be highly engaged! (Probability: {prediction_proba:.2f})")
        else:
            st.warning(f"This user is likely to have low engagement. (Probability of high engagement: {prediction_proba:.2f})")
